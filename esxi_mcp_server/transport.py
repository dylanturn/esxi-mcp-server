...
# Event to signal that the transport's connect() has set up internal streams
streamable_http_ready = asyncio.Event()
...
async def streamable_http_endpoint(scope, receive, send, mcp_server, config: Config):
            ...
            async def run_mcp_server():
                try:
                    async with streamable_http_transport.connect() as (read_stream, write_stream):
                        # Signal that the transport streams are ready for requests
                        streamable_http_ready.set()
                        init_opts = mcp_server.create_initialization_options()
                        logging.info("MCP server connected and ready for requests")
                    ...
                    # Ensure we unblock any waiters even on failure
                    streamable_http_ready.set()
    ...
    # Wait for transport streams to be initialized before handling requests
    await streamable_http_ready.wait()
